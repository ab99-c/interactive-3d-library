...
export default function GameCanvas() {
  ...
  return (
    <main className="library-game" aria-label="مكتبة 3D تفاعلية">
      ...
      <div className="hud-topline"><div className="brand-lockup"><img src="/manus-storage/library-mark_887f68d8.png" alt="" /><span>QUIET STUDY HALL</span></div><div className="status-pill"><i /> {started ? "OPEN FOR EXPLORATION" : "PREPARING THE HALL"}</div></div>
      <div className="hud-bottom"><div className="crosshair" aria-hidden="true">+</div><div className="controls"><span><b>W A S D</b> move</span><span><b>MOUSE</b> look</span><span><b>CLICK</b> inspect</span></div><button className="help-button" onClick={() => setShowHelp((value) => !value)}>{showHelp ? "Hide guide" : "Guide"}</button></div>
      {showHelp && !book && <section className="welcome-card"><div className="eyebrow">A ROOM FOR THE CURIOUS</div><h1>اختار رفاً،<br /><em>وخلي المكان يحكي.</em></h1><p>تجول بهدوء بين الرفوف. كل كتاب كيشير لحكاية مخبية فهاد القاعة.</p><button className="enter-button" onClick={() => setShowHelp(false)}>Enter the library <span>↗</span></button></section>}
