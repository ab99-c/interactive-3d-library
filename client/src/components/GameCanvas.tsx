...
export default function GameCanvas() {
  ...
  return (
    <main className="library-game" dir="rtl" aria-label="مكتبة ثلاثية الأبعاد تفاعلية">
      ...
      {started && <aside className="quest-card" aria-live="polite"><div className="quest-card-head"><span className="eyebrow">رحلة الباحث</span><span className="rank-pill">{rank.title} · {toArabicDigits(progress.xp)} نقطة</span></div>{activeObjective ? <><h2>{activeObjective.title}</h2><p>{activeObjective.description}</p><div className="quest-bar"><i style={{ width: `${activePercent}%` }} /></div><span className="quest-count">{activeProgress?.label}</span></> : <><h2>اكتملت جميع المهام</h2><p>القاعة كلها ملك لفضولك الآن — واصل القراءة والتأمل.</p></>}<div className="quest-card-foot"><span>اكتشفت {toArabicDigits(discoveredCount)} من ٠ كتاباً</span><button className="reset-progress-button" onClick={() => { const fresh = resetProgress(); progressRef.current = fresh; setProgress(fresh); }}>إعادة ضبط التقدم</button></div></aside>}
