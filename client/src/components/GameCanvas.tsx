...
export default function GameCanvas() {
  ...
  const [interactionTarget, setInteractionTarget] = useState<{ title: string; held: boolean; open: boolean; nearby: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    const onBookTarget = (event: Event) => {
      const detail = (event as CustomEvent<{ title: string; held: boolean; open: boolean; nearby?: boolean } | null>).detail;
      setInteractionTarget(detail ? { title: detail.title, held: detail.held, open: detail.open, nearby: detail.nearby !== false } : null);
    };
  ...
  return (
    <main className="library-game" dir="rtl" aria-label="مكتبة ثلاثية الأبعاد تفاعلية">
      ...
      {interactionTarget && <div className="interaction-prompt" role="status"><strong>{!interactionTarget.nearby ? "تبع الهدف · قرب للكتاب" : interactionTarget.open ? "حرّك السهمين لتقليب الصفحات" : interactionTarget.held ? "E · افتح الكتاب" : "E · خذ الكتاب"}</strong><span>{interactionTarget.title}</span></div>}
