...
## ترقيات الأداء الجديدة — مزامنة أخيرة
...
- [x] اختبار فتح الكتاب وتقليب الصفحات ومراجعة السجلات.

## طبقة اللعب — رحلة الباحث (Epic-style play layer)

- [x] إنشاء وحدة progression.ts مستقلة عن React: أهداف متسلسلة، نقاط معرفة، رتب، وحفظ في المتصفح.
- [x] ربط أحداث Babylon (book-opened، page-turned، walked) بواجهة الأهداف والإشعارات.
- [x] إضافة بطاقة الأهداف في HUD مع شريط تقدم وعداد الكتب المكتشفة وزر إعادة الضبط.
- [x] إضافة إشعارات إنجاز (toasts) مع نغمات نحاسية لكل من الاكتشاف والهدف والرتبة.
- [x] بناء نظام صوتي مولّد بالكامل: أجواء القاعة، خطوات المشي والجري، نغمات، مع كتم اختياري محفوظ.
- [x] إضافة ذرات غبار ضوئية سينمائية تُعطَّل في وضع الأداء الخفيف وprefers-reduced-motion.
- [x] إضافة خريطة مصغرة SVG للقاعة تُظهر الرفوف وموضع اللاعب.
- [x] تشغيل pnpm check وpnpm build واختبار المتصفح بدون أخطاء.
